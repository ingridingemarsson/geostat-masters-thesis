...
for i in range(0,2,2): 
	...
	#print(goes_scn.min_area())
	print(goes_scn)
	#TODO: fix resampling resolution
	ref_height = goes_scn[channel[0]].y.shape[0]
	ref_width = goes_scn[channel[0]].x.shape[0]
	goes_scn = goes_scn.aggregate(x=np.int(ref_width/width), y=np.int(ref_height/height), func='mean') # yields x = np.divide(x1, x2, out) error,mean of nan values?
	print(goes_scn)
