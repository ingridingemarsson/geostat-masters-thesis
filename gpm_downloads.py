...
gpm_file_time = gpm_extract_datetime(link_list[i])


print(gpm_data_processing(number_of_pixels, gpm_file_time, path_to_download_storage_gpm, area_def, region_corners))


#TODO: Goes downloading with cache
'''
...
filename = path_to_store_processed_data+'/datasetS'+str(gpm_file_t_0).replace(" ", "t")+'E'+str(gpm_file_t_1).replace(" ", "t")+str(channels).replace(" ", "")+'created'+str(datetime.now()).replace(" ", "")+'.nc'
